...
def search_script(script_file_name):
    ...
    # If there are fewer than 14 characters on a script, it is a teensy
    if len(script_file) < 15:
        teensy = True
...
def create_chart(file_name, status_label, printable_area):
    characters = search_script(file_name)
    ...
    page_length = 1.0
    try:
        page_length = float(printable_area)
    except ValueError:
        status_label['text'] = "Printable Area not a number!"
        return
    
    createDocument(file_name, page_length)

    status_label['text'] = "Night Order document for " + file_name + " created!"


def add_character(name, first_night_description, first_night_order_pos, other_night_description, other_night_order_pos, alignment, status_label):
    if name == "" or alignment == "" or ((first_night_description == "") and (other_night_description=="")):
        status_label['text'] = "Input was invalid, try again"
        return
    
    if first_night_order_pos != "":
        spot_to_add_character = night_one_characters.index(transform_character_name(first_night_order_pos))
        ...
        first_night_addition = [{
            ...
            'default_alignment': alignment.title()
        }, {
            "order_pos":
    ...
    if other_night_order_pos != "":
        spot_to_add_character = night_x_characters.index(transform_character_name(other_night_order_pos))
    ...
    status_label['text'] = "Character added successfully!"
...
def createDocument(filename, printable_height):
    doc = docx.Document()
